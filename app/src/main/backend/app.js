	...
	try
	{
		...
		if (!lat && !lng)
		{
			var geocodeJson = request("GET", "https://maps.googleapis.com/maps/api/geocode/json?address="
				+urlencode(street) + "," + urlencode(city) + "," + urlencode(state) + "," + urlencode(country) + "&key=" + googleAPIkey,{JSON: true});
		...
		var customSearchJson = request("GET","https://www.googleapis.com/customsearch/v1?q="+urlencode(city)+"%20City&cx="+searchEngineID+"&imgSize=huge&imgType=photo&num=8&searchType=image&key=AIzaSyDtUJvYsprcnqn7-AkWkxeN6Hu8QX3kSJ4",{JSON: true});
